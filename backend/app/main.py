...
async def lifespan(app: FastAPI) -> None:
    ...
    # Startup
    main_logger.info(
        "Starting NEWRSS application with security hardening", env=settings.ENV
    )
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        headers={"X-Error-Code": str(exc.status_code), "X-API-Version": "1.0"},
    )
...
async def connect(sid: str, environ: Dict[str, Any]) -> Optional[bool]:
    ...
    websocket_logger.info(
        "WebSocket connection established", socket_id=sid, client_ip=client_ip
    )

    # Basic connection rate limiting could be added here
    from app.core.rate_limiting import advanced_rate_limiter, RateLimitType

    is_allowed, _ = await advanced_rate_limiter.check_rate_limit(
        client_ip, RateLimitType.WEBSOCKET
    )

    if not is_allowed:
        websocket_logger.warning(
            "WebSocket connection rate limited", client_ip=client_ip
        )
